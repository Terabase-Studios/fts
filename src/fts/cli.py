...
def main():
    ...
    send_parser = subparsers.add_parser(
        "send", aliases=["s"], help="send file to target"
    )
    ...
    send_dir_parser = subparsers.add_parser(
        "send-dir", aliases=["sd", "dir"], help="send directory to target"
    )
    ...
    send_dir_parser.add_argument(
        "--py-zip", action="store_true",
        help="use python when compressing. helpful for large directories when native compression fails"
    )
    ...
    close_parser = subparsers.add_parser(
        "close", aliases=["c", "stop"], help="close detached server"
    )
    add_common_flags(close_parser)
    close_parser.set_defaults(func=lambda args, logger: __import__('fts.server').server.cmd_close(args, logger))

    # --- update ---
    update_parser = subparsers.add_parser(
        "update", aliases=["u", "upgrade"], help="update to the newest version"
    )
    update_parser.add_argument("-r", "--repair", action="store_true", help="force update")
    add_common_flags(update_parser)
    update_parser.set_defaults(func=lambda args, logger: __import__('fts.updater').updater.cmd_update(args, logger))
    ...
    trust_parser = subparsers.add_parser(
        "trust", aliases=["allow"], help="allow a new certificate to be registered to this ip"
    )
    trust_parser.add_argument("ip", help="ip address to trust")
    trust_parser.set_defaults(func=lambda args, logger: __import__('fts.secure').secure.cmd_clear_fingerprint(args, logger))
